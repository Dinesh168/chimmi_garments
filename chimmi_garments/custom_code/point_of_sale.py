...
def get_items(start, page_length, price_list, item_group, pos_profile, search_term=""):
    # Fetch warehouse and hide_unavailable_items from POS Profile
    ...
    # Search by term if search_term is provided
    if search_term:
    ...
    # Get conditions for filtering items
    condition = get_conditions(search_term)
    condition += get_item_group_condition(pos_profile)

    # Get lft and rgt for item_group
    lft, rgt = frappe.db.get_value("Item Group", item_group, ["lft", "rgt"])

    # Prepare bin join and condition for stock availability
    bin_join_selection, bin_join_condition = "", ""
    if hide_unavailable_items:
        bin_join_selection = "INNER JOIN `tabBin` bin ON bin.warehouse = %(warehouse)s AND bin.item_code = item.name AND bin.actual_qty > 0"
    ...
    items_data = frappe.db.sql(
        ...
        WHERE
            ...
            AND item.item_group IN (SELECT name FROM `tabItem Group` WHERE lft >= %(lft)s AND rgt <= %(rgt)s)
            AND {condition}
        ORDER BY
            item.name ASC
