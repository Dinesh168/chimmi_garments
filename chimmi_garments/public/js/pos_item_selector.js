...
erpnext.PointOfSale.ItemSelector = class {
		...
		this.wrapper.append(
			`<section class="items-selector">
				<div class="filter-section">
					...
					<div class="search-field" style="display: none;"></div>
	...
	render_item_list(items) {
		...
		items.forEach((item) => {
				...
				const item_html = this.get_item_html(item,saved_customer_group);
				this.$items_container.append(item_html);
		});
	}

	get_item_html(item,saved_customer_group) {
		const me = this;
		// eslint-disable-next-line no-unused-vars
		const { item_image, serial_no, batch_no, barcode, actual_qty, stock_uom, price_list_rate ,custom_school_group} = item;
		
		if (custom_school_group!==saved_customer_group)
		{
			return 
		}
